...
- **PHẦN D — delete có kiểm soát:** `deleteLibraryAction` controller thêm guard `usedByTestCases > 0 → 409 LIBRARY_IN_USE` ("Hãy bỏ Action khỏi các testcase đang dùng trước") — KHÔNG phá workspace (binding ref xóa khi block còn dùng sẽ resolve null → generate fail); unused → delete OK. UI confirm inline + disable nút Xóa khi used.
- **PHẦN F — stale/invalidation:** `GenerateService.buildFingerprint` seq giờ là `${label}|v${version}|${hash}` (hash = sha256 steps+assertions+range+label+kind) — **content change → fingerprint đổi → testcase đang dùng Action stale → Run 409 STALE_GENERATED → bắt Generate lại** (KHÔNG workaround remove/re-add). DTO listLibrary thêm `version`+`hash`.
- **Tests:** mới `automation-v3-library-edit-test.js` — A5 (rename/group persist), A6 (include-exclude step persist + hash đổi + version++), A7/A10 (delete used → 409 LIBRARY_IN_USE + message N testcase + block giữ; workspace không crash), A8 (content change → run stale 409 STALE_GENERATED → generate lại code hết step bị loại), A9 (delete unused 200 + list giảm), A11 (CodeGen + Automation cùng nguồn), A12 (workspace cũ không crash), A13 (picker bind action đã edit vẫn 200) + static A1-A4 (grid 5 cột, ellipsis, technical collapse, header actions). Cập nhật library-test (delete guard 409 + unbind trước delete; bỏ generate-422 cũ — không xảy ra qua API), library-viewer-test (edit/delete API + class grid mới). **V3+CodeGen suite 57/57 PASS + build OK + bundle mới commit.**

**P0 — LEGACY INCLUDE: UI hiện BBT nhưng runtime "Test nhap" (ĐÃ FIX 2026-08-14):**
- **Browser thật:** UI tab Chạy thử hiện Mã="BBT", runtime fill "Test nhap". TRACE code thật chứng minh: `"Test nhap"` nằm trong **`stepDecisions` legacy** (`{status:"INCLUDE", value:"Test nhap"}` — từ UI pre-6b76241 cho phép "Giữ thao tác + nhập value"), KHÔNG phải recorded/confirmed/approved. Renderer cũ: precedence STEP_DECISION INCLUDE TRƯỚC USER_CONFIRMED + `collectTestData` skip step INCLUDE → testDataMap mất Mã → fill inline legacy "Test nhap".
- **FIX (scope duyệt — không đụng UI/Library/Runner):**
  1. `resolveFillStatus` — **BỎ precedence STEP_DECISION INCLUDE**: canonical = confirmedTestData → approvedTestData (EMPTY tuyệt đối); legacy INCLUDE rơi tiếp như không có (không đọc value/intent).
  2. `collectTestData` — **KHÔNG skip step vì INCLUDE**: mọi FILL step đóng góp canonical value vào testDataMap.
  3. **HEAL deterministic** trong `autoBindTestData` (getWorkspace/generate/saveTestData): xóa MỌI `stepDecisions[].status === "INCLUDE"`; **TUYỆT ĐỐI giữ EXCLUDE**.
  4. KHÔNG fallback recordedValue; technical chưa map/canonical thiếu → 422 (tester resolve qua UI mapping).
- **Count semantics (kiểm tra đặc biệt theo yêu cầu):** sau fix, TextInput + Mã (2 FILL steps cùng map Mã) → **2 lần** fill BBT (mỗi recorded FILL step render 1 lần — KHÔNG dedupe step; đúng semantics Action); EXCLUDE TextInput → **1 lần** fill Mã. Regression assert con số chính xác, không chỉ "hết 'Test nhap'".
- **Tests:** mới `automation-v3-legacy-include-test.js` — R1 (legacy INCLUDE "Test nhap" + confirmed BBT → generated dùng BBT, hết "Test nhap"/BBC), R2 (HEAL xóa INCLUDE giữ EXCLUDE), R3 (BBT→ABC save → generate ABC, không remove/re-add/remap), R4 (Mã=EMPTY dù legacy từng chứa "Test nhap" → KHÔNG fill Mã), R5 (count semantics: 2 → EXCLUDE → 1), R1r/R5r renderer-level (legacy CHƯA heal vẫn BBT thắng + count 2/1). Cập nhật step-decision U5 (INCLUDE không còn là nguồn → UNRESOLVED). **V3+CodeGen suite 56/56 PASS + build OK + bundle mới commit.**

**P0 — UI STATE: readiness sai ("Cần review" dù data đủ) + label automation sai ("Đang thiết lập" dù đã sinh/PASSED) (ĐÃ FIX 2026-08-14):**
- **Lỗi 1:** tab Chạy thử vẫn "⚠ Cần review trước khi sinh" dù VALUE+EMPTY đủ + technical đã xử lý. **Root cause:** `actionPrepStatus` nhận `segment.inputs` (DTO không lọc EXCLUDE) + KHÔNG nhận `stepDecisions` → step EXCLUDE (VD TextInput chưa map) vẫn tính → UNRESOLVED. **Fix:** prep nhận `{steps (có order), segmentId, stepDecisions}` → SKIP `stepDecisions[segmentId:order].status === "EXCLUDE"`; technical mapped + resolved (binding → confirmed VALUE/EMPTY) → READY; fallback inputs khi không có steps.
- **Lỗi 2:** tab Thông tin vẫn "Automation: Đang thiết lập" dù Playwright đã sinh + Run Passed. **Root cause:** label chỉ dựa `segCount` (segmentSummary.total), không xét generateStatus/runStatus; card dùng contract riêng. **Fix:** helper canonical `automationDisplayStatus(testCase)` trong utils/automationV3.js — UNDECIDED+chưa action → "Chưa thiết lập"; AUTOMATED+chưa generated → "Đang thiết lập"; generated+NOT_RUN → "Đã sinh automation"; generated+PASSED → "Automation sẵn sàng"; generated+FAILED → "Có automation · Chạy thử thất bại". Dùng CHUNG: drawer header + tab Thông tin + card (thêm row "Automation: X").
- KHÔNG đổi backend/canonical semantics/Runner; không hard-code.
- **Tests:** mới `automation-v3-ui-state-test.js` — R1 (VALUE+EMPTY + technical xử lý → READY), R2 (EXCLUDE step không pending; fallback inputs vẫn review — chứng minh fix), R3 (mapped technical + resolved → READY), R4-R6 (label canonical 5 trạng thái), R7 (card+drawer cùng helper — static), R8 (reopen cùng state → cùng label), R9 (run rows state đúng). **V3+CodeGen suite 55/55 PASS + build OK + bundle mới commit.**
