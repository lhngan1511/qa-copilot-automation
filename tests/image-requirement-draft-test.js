...
const inferredPreconditionProvider = { async analyzeRequirementImages() { return { document: { ...document, features: [{ ...document.features[0], preconditions: ["Chưa xác định.", "Người dùng đã đăng nhập và truy cập trang Đơn vị tính.", "Đơn vị tính cần tìm đã tồn tại."] }] } }; } };
const inferredPreconditionService = new ImageRequirementDraftService({ dataDir: path.join(temp, "preconditions"), provider: inferredPreconditionProvider, requirementUploadService: upload });
const inferredPreconditionDraft = await inferredPreconditionService.analyze({ projectId: "PRJ-A", images: [image] });
const inferredPreconditions = new MarkdownParser().parse(inferredPreconditionDraft.markdownContent).features[0].preconditions;
assert.deepEqual(inferredPreconditions, ["Đơn vị tính cần tìm đã tồn tại."], "image flow removes unknown and generic login/access preconditions but keeps concrete business state");
assert.doesNotMatch(inferredPreconditionDraft.markdownContent, /### Điều kiện tiên quyết\s*\n\s*- Chưa xác định/i, "empty image preconditions do not render placeholder");
