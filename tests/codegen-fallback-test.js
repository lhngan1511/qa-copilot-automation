...
async function main() {
    ...
    const spec = buildSpecFromMapping({ testCase, mapping });
    ...
    assert.ok(spec.includes("process.env.LOGIN_USERNAME"), "Tài khoản -> env (không hardcode)");
    ...
    assert.equal(renderStepLine({ actionType: "FILL", target: "Tài khoản", locator: "page.getByLabel('Tài khoản')" }), "  await page.getByLabel('Tài khoản').fill(\"process.env.LOGIN_USERNAME\");");
    const setup = setupPrefixLines(mapping);
    ...
    const gen = await svc.generate({ testCase, mapping, codegenText: "x", confirmedFacts: [] });
