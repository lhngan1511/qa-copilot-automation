...
// D — không binding -> fallback recorded theo contract (không đoán)
const save3 = await req("PATCH", `/api/automation-v3/workspaces/${wid}/testcases/TC008/test-data`, { testData: {}, bindings: {} });
const genD = await req("POST", `/api/automation-v3/workspaces/${wid}/testcases/TC008/generate`, {});
...
assert.ok(dClean.includes("tdBindings") && dClean.includes("mappedTargets"), "F: editor có binding state + mapped targets");
assert.ok(dClean.includes("chọn input của thao tác"), "F: select binding business field ↔ input");
assert.ok(dClean.includes("kỹ thuật (chưa map business field)"), "F: action input chưa map hiển thị nhãn kỹ thuật");
