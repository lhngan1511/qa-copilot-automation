...
export function classifyTestCaseIntent(testCase = {}) {
    ...
    if (operation === "CREATE") {
        if (isAutoGeneratedCode(testCase)) {
...
function isAutoGeneratedCode(testCase = {}) {
    const classification = key(testCase.ruleClassification);
    if (classification === "auto generated") return true;
    const titleText = key([testCase.title, testCase.scenario, testCase.testScenario].join(" "));
    const expected = key(testCase.expectedResult);
    const mentionsCode = /(?:^| )ma(?: |$)|ma don vi|ma ho so|ma thiet bi|\bcode\b/.test(
        `${titleText} ${expected}`
    );
    if (/tu sinh|tu dong sinh/.test(expected) && mentionsCode) return true;
    if (
        /khong nhap ma|de trong ma|bo trong ma/.test(titleText) &&
        !/khong (duoc|cho phep)|bat buoc/.test(titleText)
    ) {
        return true;
    }
    return false;
...
export function presentTestCases(testCases = []) {
    ...
    return assignDisplayIds(decorated);
}
...
export function applyTestCasePresentation(testCases = []) {
    return presentTestCases(testCases);
}
