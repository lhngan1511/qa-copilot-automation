...
export default class RequirementReviewWorkflow extends BaseReviewWorkflow {
    ...
    start(context = {}) {
        ...
        const updatedSession = {
            ...session,
            workflowName: session.workflowName ?? "requirement-review",
            status: "started",
        ...
        if (artifact) {
            this.artifactRepository.save({
                ...
                artifactType: artifact.artifactType ?? "requirement"
            });
        }
    ...
    complete(context = {}) {
        ...
        if (session.status !== "approved") {
            throw new Error("Requirement review must be approved before completion.");
        }
    ...
    resolveArtifact(context = {}) {
        const source = context.artifact ?? context.requirement ?? null;
