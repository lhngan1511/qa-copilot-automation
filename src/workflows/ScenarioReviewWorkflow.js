...
export default class ScenarioReviewWorkflow extends BaseReviewWorkflow {
    ...
    start(context = {}) {
        ...
        const updatedSession = {
            ...session,
            workflowName: session.workflowName ?? "scenario-review",
            status: "started",
        ...
        if (artifact) {
            this.artifactRepository.save({
                ...
                artifactType: artifact.artifactType ?? "scenario"
            });
        }
    ...
    complete(context = {}) {
        ...
        if (session.status !== "approved") {
            throw new Error("Scenario review must be approved before completion.");
        }
    ...
    resolveArtifact(context = {}) {
        const source = context.artifact ?? context.scenario ?? null;
