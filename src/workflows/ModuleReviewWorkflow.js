...
export default class ModuleReviewWorkflow extends BaseReviewWorkflow {
    ...
    start(context = {}) {
        ...
        const updatedSession = {
            ...session,
            workflowName: session.workflowName ?? "module-review",
            status: "started",
        ...
        if (artifact) {
            this.artifactRepository.save({
                ...
                artifactType: artifact.artifactType ?? "module"
            });
        }
    ...
    complete(context = {}) {
        ...
        if (session.status !== "approved") {
            throw new Error("Module review must be approved before completion.");
        }
    ...
    resolveArtifact(context = {}) {
        const source = context.artifact ?? context.module ?? null;
