...
export default class RuleTestDataBuilder {
    classify({ rule = "", source = "", scenario = {} } = {}) {
        ...
        if (
            /(?:để trống|không nhập).*(?:tự sinh|tự động sinh|hệ thống sinh)|(?:tự sinh|tự động sinh|hệ thống sinh).*(?:khi|nếu).*(?:để trống|không nhập)/.test(
                text
            )
        ) {
    ...
    build({ scenario = {}, sourceItem = {}, existingTestData = {} } = {}) {
        ...
        switch (classification) {
            ...
            case "AUTO_GENERATED":
                if (this.isSpecificField(fieldName)) {
                    testData.valid[fieldName] = "";
                    testData.context.autoGeneratedField = fieldName;
                    testData.expectedState.autoGenerated = true;
    ...
    buildExpected({ classification, fieldName, rule, scenario, flags }) {
        ...
        switch (classification) {
            ...
            case "AUTO_GENERATED":
                return fieldName
                    ? `Hệ thống tự sinh ${fieldName} khi người dùng để trống và lưu bản ghi thành công; giá trị được sinh không được trùng với bản ghi hiện có.`
                    : "Hệ thống tự sinh giá trị theo quy tắc đã được xác nhận và lưu bản ghi thành công.";
            case "DUPLICATE":
    ...
    buildTrigger(classification, fieldName, rule, testData, scenario = {}) {
        const actions = {
            ...
            AUTO_GENERATED: `Để trống ${fieldName} để hệ thống tự sinh`,
        ...
        return {
            ...
            value:
                ["REQUIRED", "AUTO_GENERATED"].includes(classification)
                    ? ""
    ...
    extractFieldName(rule, classification) {
        if (classification === "AUTO_GENERATED") {
            const text = this.businessText(rule);
            const generatedFirst = text.match(
                /^(.+?)\s+(?:hệ thống\s+)?(?:tự sinh|tự động sinh|sinh tự động).*(?:để trống|không nhập)/i
            );
            if (generatedFirst?.[1]) return generatedFirst[1].trim();

            const blankFirst = text.match(
                /^(.+?)(?:\s*:\s*|\s+)(?:nếu|khi)?\s*(?:để trống|không nhập).*(?:tự sinh|tự động sinh|hệ thống sinh)/i
            );
            if (blankFirst?.[1]) return blankFirst[1].trim();
        }

        const patterns = {
            REQUIRED: /^(.+?)\s+(?:không được để trống|bị bỏ trống|phải được nhập|required)\b/i,
            AUTO_GENERATED:
                /^(.+?)(?:\s*:\s*|\s+)(?:(?:nếu|khi)\s+)?(?:để trống|không nhập).*(?:tự sinh|tự động sinh|hệ thống sinh)|^(.+?)(?:\s*:\s*|\s+)(?:tự sinh|tự động sinh|hệ thống sinh).*(?:để trống|không nhập)/i,
            DUPLICATE:
        ...
        const match = this.businessText(rule).match(patterns[classification] ?? /^$/);
