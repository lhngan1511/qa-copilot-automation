...
export default class CodeGenController {
    ...
    async listLibrary(req, res) {
        try {
            if (!this.actionLibrary)
                return this.fail(
                    res,
                    new Error("ActionLibrary chưa cấu hình."),
                    500,
                    "LIBRARY_NOT_CONFIGURED",
                    "Thư viện thao tác chưa sẵn sàng."
                );
            const usage =
            ...
            return this.fail(
                ...
                500,
                "LIBRARY_LIST_FAILED",
                "Không đọc được thư viện thao tác."
            );
        }
    }
    ...
    async createLibraryAction(req, res) {
        try {
            ...
            if (!recordingId || !Number.isInteger(startStep) || !Number.isInteger(endStep)) {
                return this.fail(
                    res,
                    new Error("Thiếu recordingId/startStep/endStep."),
                    400,
                    "INVALID_REQUEST",
                    "Thiếu thông tin đoạn thao tác."
                );
            }
            ...
            if (!trimmedLabel)
                return this.fail(
                    res,
                    new Error("Thiếu tên thao tác."),
                    400,
                    "LIBRARY_LABEL_REQUIRED",
                    "Tên thao tác bắt buộc."
                );
            const rec = this.manager.get(recordingId);
            if (!rec)
                return this.fail(
                    res,
                    new Error("Không tìm thấy bản ghi."),
                    404,
                    "RECORDING_NOT_FOUND",
                    "Không tìm thấy bản ghi."
                );
            const steps = (rec.steps ?? []).filter(
                s => Number.isInteger(s?.order) && s.order >= startStep && s.order <= endStep
            );
            if (steps.length === 0)
                return this.fail(
                    res,
                    new Error("Khoảng bước không có thao tác."),
                    400,
                    "INVALID_RANGE",
                    "Khoảng bước không hợp lệ."
                );
            ...
            const asserts = (rec.assertions ?? [])
                .filter(a => {
                    ...
                    if (as >= (first?.sourceStart ?? 0) && ae <= (last?.sourceEnd ?? 0))
                        return true;
                    if (as >= (last?.sourceStart ?? 0) && as <= (last?.sourceEnd ?? 0) + 120)
                        return true;
                    return false;
                })
                .map(a => ({
            ...
            return res.status(201).json({
                ...
                data: {
                    ...
                    recordedAssertionCount: block.recordedAssertions.length
                },
                error: null
            });
        } catch (error) {
            return this.fail(
                ...
                "Không tạo được thao tác thư viện."
            );
        }
    }
    ...
    async deleteLibraryAction(req, res) {
        try {
            ...
            if (!this.actionLibrary.belongsToProject(blockId, projectId)) {
                return this.fail(
                    res,
                    new Error("Không tìm thấy thao tác."),
                    404,
                    "LIBRARY_BLOCK_NOT_FOUND",
            ...
            if (!removed)
                return this.fail(
                    ...
                    404,
                    "LIBRARY_BLOCK_NOT_FOUND",
                    "Không tìm thấy thao tác trong thư viện."
                );
            return res
                .status(200)
            ...
            return this.fail(
                ...
                "Không xóa được thao tác thư viện."
            );
        }
    }
        ...
        try {
            ...
            if (steps !== undefined) {
                if (!Array.isArray(steps))
                    return this.fail(
                        res,
                        new Error("steps phải là mảng."),
                        400,
                        "INVALID_REQUEST",
                        "Danh sách bước không hợp lệ."
                    );
                ...
                patch.steps = steps.map(s => {
                    const original =
                        (current?.steps ?? []).find(item => item.order === s.order) ?? {};
                    const incoming = { ...s };
                    const preserveSensitive = Boolean(
                        original.sensitive && incoming.preserveRecordedValue
                    );
                    ...
                    return {
                        ...original,
                        ...incoming,
                        recordedValue: preserveSensitive
                            ? original.recordedValue
                            : incoming.recordedValue,
            ...
            try {
                ...
                return this.fail(
                    ...
                    e,
                    400,
                    e.code ?? "LIBRARY_UPDATE_FAILED",
                    e.message ?? "Không cập nhật được thao tác."
                );
            }
            if (!updated)
                return this.fail(
            ...
            return this.fail(
                ...
                "Không cập nhật được thao tác thư viện."
            );
        }
    }
        ...
        try {
            ...
            try {
                ...
                return this.fail(
                    ...
                    e,
                    400,
                    e.code ?? "GROUP_RENAME_FAILED",
                    e.message ?? "Không đổi được tên nhóm."
                );
            }
    ...
    async analyzeRecording(req, res) {
        try {
            ...
            const assertions = (rec.assertions ?? []).map(a => ({
                order: a.order,
                statement: a.statement ?? "",
                matcher: a.matcher ?? "",
                locator: a.locator ?? ""
            ...
            if (steps.length === 0)
                return res
                    .status(200)
                    .json({ success: true, data: { proposals: [] }, error: null });
            let provider = null;
            ...
            if (!provider) {
                return res.status(200).json({
                    ...
                    error: {
                        ...
                        retryable: true,
                        message: "AI provider chưa sẵn sàng."
                    }
                });
            }
...
- KHÔNG được gom nhiều validation chỉ vì chúng cùng loại lỗi chung như "thiếu thông tin", "sai thông tin", "không hợp lệ".
- Ví dụ với chức năng Đăng nhập:
  + Bỏ trống Tài khoản + assertion "Vui lòng nhập Tên tài khoản" = một proposal riêng.
  + Bỏ trống Mật khẩu + assertion "Vui lòng nhập Mật khẩu" = một proposal riêng.
  + Bỏ trống Mã xác nhận + assertion "Vui lòng nhập Mã xác nhận" = một proposal riêng.
  + Sai tài khoản hoặc mật khẩu + assertion lỗi xác thực = một proposal riêng.
- Khi nhiều assertion khác nhau xuất hiện ở các thời điểm khác nhau trong recording, ưu tiên dùng từng assertion làm mốc kết thúc của proposal tương ứng.
- Mỗi proposal phải đại diện cho đúng MỘT business intent có thể thực thi độc lập sau khi bổ sung setup cần thiết.
...
assertions: ${JSON.stringify(assertions)}`;
            ...
            try {
                ...
                return res.status(200).json({
                    ...
                    error: {
                        ...
                        retryable: true,
                        message: "AI trả về không đúng định dạng."
                    }
                });
            }
    ...
    async start(req, res) {
        try {
            ...
            return this.fail(
                ...
                error,
                200,
                "CODE_GEN_START_FAILED",
                "Không thể bắt đầu ghi CodeGen."
            );
        }
    }
    ...
    async stop(req, res) {
        try {
            ...
            return this.fail(
                ...
                error,
                409,
                "CODE_GEN_STOP_FAILED",
                "Không thể dừng phiên ghi CodeGen."
            );
        }
    }
    ...
    async setScript(req, res) {
        try {
            ...
            return this.fail(
                ...
                error,
                200,
                "CODE_GEN_SET_SCRIPT_FAILED",
                "Không thể lưu script."
            );
        }
    }
    ...
    async list(req, res) {
        try {
            ...
            return this.fail(
                ...
                error,
                500,
                "CODE_GEN_LIST_FAILED",
                "Không thể đọc danh sách recording."
            );
        }
    }
            ...
            if (!data || String(data.projectId ?? "") !== String(projectId ?? ""))
                return res.status(404).json({
                    ...
                    error: {
                        ...
                        message: "Không tìm thấy recording trong Project hiện tại."
                    }
                });
            return res.status(200).json({ success: true, data, error: null });
        } catch (error) {
    ...
    async testcases(req, res) {
        try {
            ...
            const hasContext = recordingId ? this.manager.hasReliableContext(recordingId) : false;
            let testcases = [];
            ...
            return res.status(200).json({
                ...
                data: {
                    enabled: hasContext,
                    context: recordingId ? this.manager.getContext(recordingId) : null,
                    testcases
                },
                error: null
            });
            ...
            return this.fail(
                ...
                error,
                500,
                "CODE_GEN_TESTCASES_FAILED",
                "Không thể đọc danh sách testcase."
            );
        }
    }
    ...
    filterTestcasesByContext(testcases, ctx = {}) {
        const ctxModule = String(ctx.module ?? "")
            .trim()
            .toLowerCase();
        const ctxFeature = String(ctx.feature ?? "")
            .trim()
            .toLowerCase();
        const ctxModuleId = String(ctx.moduleId ?? "")
            .trim()
            .toLowerCase();
        const ctxFunctionId = String(ctx.functionId ?? "")
            .trim()
            .toLowerCase();
        return (Array.isArray(testcases) ? testcases : []).filter(tc => {
            const review = String(tc.reviewStatus ?? "")
                .trim()
                .toUpperCase();
            ...
            const m = String(tc.module ?? "")
                .trim()
                .toLowerCase();
            const f = String(tc.feature ?? "")
                .trim()
                .toLowerCase();
            const mi = String(tc.moduleId ?? "")
                .trim()
                .toLowerCase();
            const fi = String(tc.functionId ?? "")
                .trim()
                .toLowerCase();
            // khớp nếu context module/feature/id trùng
            const matchModule =
                !ctxModule || (m && m.includes(ctxModule)) || (ctxModule && ctxModule.includes(m));
            const matchFeature =
                !ctxFeature ||
                (f && f.includes(ctxFeature)) ||
                (ctxFeature && ctxFeature.includes(f));
    ...
    async setContext(req, res) {
        try {
            ...
            return this.fail(
                ...
                error,
                200,
                "CODE_GEN_SET_CONTEXT_FAILED",
                "Không thể gán context."
            );
        }
    }
    ...
    async status(req, res) {
        try {
            ...
            return this.fail(
                ...
                error,
                500,
                "CODE_GEN_STATUS_FAILED",
                "Không thể đọc trạng thái CodeGen."
            );
        }
    }
    ...
    async focus(req, res) {
        try {
            ...
            return this.fail(
                ...
                error,
                200,
                "CODE_GEN_FOCUS_FAILED",
                "Không thể focus cửa sổ ghi."
            );
        }
    }
    ...
    async cleanup(req, res) {
        try {
            ...
            return this.fail(
                ...
                error,
                500,
                "CODE_GEN_CLEANUP_FAILED",
                "Không thể dọn dữ liệu CodeGen."
            );
        }
    }
